...
function group(array, keySelector, valueSelector) {
  ...
  keys.forEach((key) => {
    const values = array.filter((element) => keySelector(element) === key);
    const pureValues = values.map((el) => valueSelector(el));
    result.set(key, pureValues);
  });
